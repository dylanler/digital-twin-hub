...
def generate_metadata_without_environment(num_scenes, script, model="gemini", video_engine="luma", lora_in_first_frame_mode=False, trigger_word=None, lora_inference=None):
    ...
    if lora_in_first_frame_mode:
        # Get environment and object trigger words from the lora_inference object if available
        env_trigger = getattr(lora_inference, 'environment_trigger', None) if lora_inference else None
        obj_trigger = getattr(lora_inference, 'object_trigger', None) if lora_inference else None
        
        prompt += f'\n\nIMPORTANT INSTRUCTIONS FOR FIRST FRAME PROMPTS:'
        prompt += f'\n1. You MUST include ALL of these trigger words in EVERY first_frame_prompt:'
        prompt += f'\n   - Character: "{trigger_word}" (REQUIRED)'
        if env_trigger:
            prompt += f'\n   - Environment: "{env_trigger}" (REQUIRED)'
        if obj_trigger:
            prompt += f'\n   - Object: "{obj_trigger}" (REQUIRED)'
        
        prompt += f'\n2. Rules for using trigger words:'
        prompt += f'\n   - The character trigger word "{trigger_word}" should appear EXACTLY ONCE in each first_frame_prompt'
        prompt += f'\n   - Use trigger words EXACTLY as provided, do not modify them'
        prompt += f'\n   - Combine trigger words naturally in a coherent scene description'
        
        prompt += f'\n3. Example first frame prompts:'
        if env_trigger and obj_trigger:
            prompt += f'\n   - "{trigger_word} exploring a {env_trigger} while holding a {obj_trigger}"'
            prompt += f'\n   - "In a mystical {env_trigger}, {trigger_word} examines a glowing {obj_trigger}"'
        elif env_trigger:
            prompt += f'\n   - "{trigger_word} walking through a beautiful {env_trigger}"'
            prompt += f'\n   - "A serene {env_trigger} where {trigger_word} stands contemplatively"'
        elif obj_trigger:
            prompt += f'\n   - "{trigger_word} proudly displaying a {obj_trigger}"'
            prompt += f'\n   - "Close-up of {trigger_word} interacting with a {obj_trigger}"'
        
        prompt += f'\n4. EVERY first_frame_prompt MUST be validated to contain ALL required trigger words'
    ...
    try:
        if model == "gemini":
            ...
            IMPORTANT: When LoRA trigger words are provided, you MUST include ALL of them in EVERY first_frame_prompt.
            """
            ...
            response = client.messages.create(
                ...
                Remember: EVERY first_frame_prompt MUST include ALL provided trigger words.
                """}]
            )
        ...
        # If lora_in_first_frame_mode is enabled, ensure all trigger words are in each first_frame_prompt
        if lora_in_first_frame_mode:
            for scene in metadata:
                # Check if all required trigger words are present
                missing_triggers = []
                if trigger_word not in scene['first_frame_prompt']:
                    missing_triggers.append(trigger_word)
                if env_trigger and env_trigger not in scene['first_frame_prompt']:
                    missing_triggers.append(env_trigger)
                if obj_trigger and obj_trigger not in scene['first_frame_prompt']:
                    missing_triggers.append(obj_trigger)
                
                # If any triggers are missing, reconstruct the prompt
                if missing_triggers:
                    print(f"Fixing missing trigger words in scene {scene['scene_number']}: {missing_triggers}")
                    # Start with the character in the environment
                    if env_trigger:
                        base_prompt = f"{trigger_word} in a majestic {env_trigger}"
                    else:
                        base_prompt = f"{trigger_word} in a beautiful setting"
                    
                    # Add the object if it exists
                    if obj_trigger:
                        base_prompt += f", holding a {obj_trigger}"
                    
                    # Combine with original prompt while avoiding repetition
                    original_prompt = scene['first_frame_prompt']
                    # Remove any existing mentions of trigger words to avoid duplication
                    for word in [trigger_word, env_trigger, obj_trigger]:
                        if word:
                            original_prompt = original_prompt.replace(word, "").strip()
                    # Clean up any double spaces or punctuation
                    original_prompt = ' '.join(original_prompt.split())
                    
                    # Combine prompts if the original has useful additional details
                    if original_prompt and not original_prompt.isspace():
                        scene['first_frame_prompt'] = f"{base_prompt}, {original_prompt}"
                    else:
                        scene['first_frame_prompt'] = base_prompt
...
def generate_scenes(scenes, video_engine="luma", skip_sound_effects=False, initial_image_path=None, initial_image_prompt=None, lora_in_first_frame_mode=False, lora_inference=None, lora_path=None, ltx_model_args=None):
    ...
    Args:
        ...
        lora_inference (Args): Arguments object for multi-LoRA inference
        lora_path (str): Path to the main character LoRA weights file
    ...
    if lora_in_first_frame_mode:
        ...
        # Import multi_lora_inference here to avoid circular imports
    ...
    for i, scene in enumerate(scenes):
        ...
        if lora_in_first_frame_mode:
            # Generate first frame using multi-LoRA inference
            try:
                first_frame_path = os.path.join(scene_dir, f"scene_{scene['scene_number']}_first_frame.jpg")
                
                # Set up arguments for multi-LoRA inference
                lora_inference.output_name = first_frame_path
                lora_inference.prompt_template = scene['first_frame_prompt']
                
                print(f"\nGenerating first frame for scene {scene['scene_number']} using multi-LoRA")
                print(f"Prompt: {scene['first_frame_prompt']}")
                print(f"Output path: {first_frame_path}")
                
                # Run multi-LoRA inference
                multi_lora_main(lora_inference)
                
                # Verify the image was generated and upload it
                if os.path.exists(first_frame_path):
                    if os.path.getsize(first_frame_path) > 0:
                        scene_start_image_url = uploader.upload_image(first_frame_path)
                        print(f"Successfully generated and uploaded first frame: {scene_start_image_url}")
                        
                        # Store the URL of the first frame of the first scene
                        if scene['scene_number'] == 1:
                            first_frame_of_first_scene_url = scene_start_image_url
                    else:
                        print(f"Warning: Generated first frame is empty for scene {scene['scene_number']}")
                else:
                    print(f"Warning: Failed to generate first frame at path: {first_frame_path}")
            except Exception as e:
                print(f"Error generating first frame for scene {scene['scene_number']}: {str(e)}")
                traceback.print_exc()
